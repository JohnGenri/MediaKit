...
* **`download_instagram.sh.example`**: **Этот файл — шаблон. Бот использует файл `download_instagram.sh`, который по умолчанию игнорируется Git (через `.gitignore`) для защиты ваших данных. Не забудьте скопировать `.example` в `.sh` и вписать туда свой прокси.**
